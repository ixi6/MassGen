...
def _make_artifact_payload() -> str:
    """
    Create a base64-encoded tar.gz archive containing a single file named "events.jsonl" with the bytes "hello from cloud".
    
    Returns:
        base64_payload (str): Base64-encoded string of the tar.gz archive described above.
    """
    buf = io.BytesIO()
...
def _make_mock_popen(stdout_lines: list[str], stderr_lines: list[str], returncode: int):
    """
    Create a mock subprocess.Popen-like object that simulates stdout, stderr, and exit behavior for tests.
    
    Parameters:
        stdout_lines (list[str]): Lines to yield from the mock's stdout iterator.
        stderr_lines (list[str]): Lines to yield from the mock's stderr iterator.
        returncode (int): Exit code returned by wait() and assigned to `.returncode`.
    
    Returns:
        MagicMock: Mock process with `.stdout` and `.stderr` as iterators over the provided lines, `.returncode` set to `returncode`, and `.wait()` returning `returncode`.
    """
    mock_proc = MagicMock()
...
def test_process_context_paths_files_and_directories(tmp_path, monkeypatch):
    ...
    class FakeVolume:
        ...
        def from_name(cls, name, create_if_missing=False):
            """
            Locate a Volume by its name, optionally creating it if it does not exist.
            
            Parameters:
                name (str): The name of the volume to look up.
                create_if_missing (bool): If True, create the volume when it cannot be found.
            
            Returns:
                Volume: The Volume instance corresponding to `name`.
            """
            return mock_volume
