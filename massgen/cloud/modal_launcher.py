...
class ModalCloudJobLauncher(CloudJobLauncher):
    ...
    def __init__(self, workspace_root: Path | None = None):
        """
        Initialize the ModalCloudJobLauncher with an optional workspace root.
        
        Parameters:
            workspace_root (Path | None): Filesystem path where job artifacts and workspace will be created. If `None`, the launcher uses the default workspace root provided by the base CloudJobLauncher.
        """
        super().__init__(workspace_root)

    def launch(self, request: CloudJobRequest) -> CloudJobResult:
        """
        Launches a MassGen cloud job via Modal, waits for its result marker, and materializes returned artifacts into the workspace.
        
        Runs the Modal entrypoint with a payload derived from `request`, streams the job output while capturing a result marker JSON, raises CloudJobError on startup failures, timeouts, or execution errors reported by the cloud job, and extracts any returned artifacts into a per-job artifacts directory.
        
        Parameters:
            request (CloudJobRequest): Cloud job request containing the prompt, configuration YAML, timeout, and a unique `cloud_job_id` used to correlate the result.
        
        Returns:
            CloudJobResult: Result object containing `final_answer`, the local `artifacts_dir`, optional `local_log_dir`, optional `local_events_path`, and `remote_log_dir` reported by the cloud job.
        """
        ...
        def _drain_stderr():
            """
            Continuously read lines from the subprocess stderr stream and append each raw line to the enclosing `stderr_lines` list.
            
            Reads from `proc.stderr` until EOF; the function asserts that `proc.stderr` is present before draining.
            """
